...
function boundedHarnessFailureCode(failureCode: ProviderGenerationValidationProjection['failureCode']): EntrypointFailureCode {
    if (failureCode === null) {
        return null;
    }
...
async function concretePreflight(phase: 'PRE' | 'POST', input: Invocation, deps: ExecutionDependencies, fileSystem: NodeOllamaPreflightFileSystem): Promise<GenerationInventorySnapshot> {
    void phase;
    const runner = new ContainedOllamaPreflightProcessRunner((command, args, options) => spawn(command, [...args], options), createRunnerOwnedOllamaSandbox);
