...
import {
  ...
  PROVIDER_GENERATION_VALIDATION_CONTRACT_VERSION,
...
export interface EntrypointProjection {
  ...
  readonly failureCode: EntrypointFailureCode;
  readonly promptDigest: typeof VALIDATION_PROMPT_DIGEST;
  readonly harnessInvocationCount: number;
  readonly identityVerified: boolean;
  readonly selectedProviderId: string | null;
  readonly selectedAdapterId: string | null;
  readonly selectedModelId: string | null;
  readonly planAttemptCount: number;
...
export function parseProviderGenerationExecutionInvocation(argv: readonly string[]): Invocation {
  ...
  const executableRealpath = values.get(FLAGS[0]);
  const hash = values.get(FLAGS[1]);
  const sizeText = values.get(FLAGS[2]);
  const endpointInput = values.get(FLAGS[3]);
  const acquisition = values.get(FLAGS[4]);
  const egress = values.get(FLAGS[5]);
  if (
    executableRealpath === undefined || hash === undefined || sizeText === undefined ||
    endpointInput === undefined ||
    acquisition === undefined || egress === undefined || !isAbsolute(executableRealpath) ||
    !/^[0-9a-f]{64}$/.test(hash) || !/^[1-9][0-9]*$/.test(sizeText)) {
    invalid();
  }
  ...
  const endpoint = (() => {
    try {
      ...
      if (parsedEndpoint.hostname !== '127.0.0.1') {
        invalid();
      }
  ...
  if (!Object.values(ModelAcquisitionControl).includes(acquisition as ModelAcquisitionControl) ||
    !Object.values(ExternalEgressControl).includes(egress as ExternalEgressControl)) {
    invalid();
  }
...
export interface ExecutionDependencies {
  readonly fileSystem?: NodeOllamaPreflightFileSystem;
  readonly executeHarness?: Harness;
  readonly runPreflight?: (phase: 'PRE' | 'POST', input: Invocation) => Promise<GenerationInventorySnapshot>;
  readonly writeProjection?: (value: string) => void;
...
interface Lifecycle {
  ...
  projectionEmissionAttemptCount: number;
  projectionEmissionSucceeded: boolean;
}
...
function exitCodeFor(status: Status): 0 | 2 | 3 {
  switch (status) {
    case 'PASS':
      return 0;
    case 'FAIL':
...
function approved(input: Invocation): ApprovedOllamaExecutable {
  return Object.freeze({ realPath: input.executableRealpath,
...
async function concretePreflight(
  phase: 'PRE' | 'POST',
  input: Invocation,
  deps: ExecutionDependencies,
  fileSystem: NodeOllamaPreflightFileSystem,
): Promise<GenerationInventorySnapshot> {
  void phase;
  ...
  const result = await new OllamaInventoryPreflight(fileSystem, runner).execute({
    executablePath: input.executableRealpath,
...
export async function executeProviderGenerationExecution(argv: readonly string[], deps: ExecutionDependencies = {}) {
  ...
  const finish = (projection: EntrypointProjection, exitCode: number) => {
    ...
    try {
      writer(JSON.stringify(projection));
      lifecycle.projectionEmissionSucceeded = true;
      return { exitCode, projection, lifecycle: Object.freeze({ ...lifecycle }) };
    }
    catch {
      return { exitCode: 5, projection: projectionOf(lifecycle, 'BLOCKED', 'UNEXPECTED_ENTRYPOINT_FAILURE'),
        lifecycle: Object.freeze({ ...lifecycle }) };
    }
  };
  let input: Invocation;
  try {
    input = parseProviderGenerationExecutionInvocation(argv);
  }
  catch {
    return finish(projectionOf(lifecycle, 'BLOCKED', 'INVALID_INVOCATION'), 4);
  }
  ...
  try {
    assertOllamaExecutableIdentity(approved(input), input.executableRealpath, fileSystem);
    lifecycle.identityVerified = true;
  }
  catch {
    return finish(projectionOf(lifecycle, 'BLOCKED', 'EXECUTABLE_IDENTITY_MISMATCH'), 3);
  }
  try {
    const harness = deps.executeHarness ?? executeProviderGenerationValidation;
    const runPreflight = deps.runPreflight ?? (
      (phase: 'PRE' | 'POST') => concretePreflight(phase, input, deps, fileSystem)
    );
    const harnessDependencies: ProviderGenerationValidationDependencies = {
      runPreflight: (phase) => runPreflight(phase, input),
    ...
    return finish(projectionOf(lifecycle, status, failureCode), exitCodeFor(status));
  }
  catch {
    return finish(projectionOf(lifecycle, 'BLOCKED', 'UNEXPECTED_ENTRYPOINT_FAILURE'), 5);
  }
}
if (require.main === module) {
  void executeProviderGenerationExecution(process.argv.slice(2)).then(({ exitCode }) => {
