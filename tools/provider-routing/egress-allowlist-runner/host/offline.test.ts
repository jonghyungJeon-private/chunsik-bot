...
describe('terminal evidence, dependencies, and mappings', () => {
  ...
  it('rejects a resolved record mismatch against its authoritative resolution before dispatch', () => { let calls = 0;
    const resolvedContract = { ...STATIC.contract, records: STATIC.contract.records.map((record, index) =>
      index === 0 ? { ...record, timeoutMs: record.timeoutMs + 1 } : record) };
    const result = run({ dispatch: () => { calls += 1; } }, { resolvedContract,
