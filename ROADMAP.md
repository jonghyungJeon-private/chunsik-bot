...
| **M3E-6G** | TaskRun persistence safety | IMPLEMENTED LOCALLY / AWAITING REVIEW; bound TaskRun delete prohibition (all statuses incl. terminal history) enforced from persisted task_id + canonical binding inside one IMMEDIATE transaction; explicit SQLite lock wait (5000 ms default, validated); typed `TASK_RUN_STORAGE_BUSY` distinct from `UNRESOLVED_STARTED_RUN`; CANCELED/revival coverage; unbound delete and missing-id no-op preserved; no schema/migration/status/repository; automatic Application retry NO; raw-SQL immunity NOT claimed; activation DISABLED |
| **M3E-6H → M3E-6L** | Remaining ratified activation prerequisite slices | 6H static AgentProfile config · 6I-a shared pure live-plan predicate → **Product Decision gate** (trigger, authorized Actor/Project scope, supported receiver capability set) → 6I-b post-wait context contract → 6J explicit caller preparation → 6K receiver seam + exact-run terminalization → 6L offline activation acceptance; 6J must not precede 6I-b; live activation needs separate strict approval |
