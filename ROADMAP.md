...
| **M3E-6F** | Activation readiness architecture | ADR-0089 Proposed; docs-only; activation NO; deletion, contention, config, operation scope, explicit Product trigger and same-call receiver prerequisites; no runtime/worker |
| **Proposed M3E-6G–6L** | Bounded activation prerequisites | After ADR-0089 ratification: persistence safety → static config → plan/scope proof → Product-selected caller → receiver/terminalization → offline acceptance; separate strict approval for any live activation |
