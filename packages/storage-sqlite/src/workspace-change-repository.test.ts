...
import { ApprovalStatus, PatchStatus, WorkspaceChangeStatus } from '@quoky/core';
import type { WorkspaceChange } from '@quoky/core';
...
async function freshStore(): Promise<SqliteStorageProvider> {
  const dir = mkdtempSync(join(tmpdir(), 'quoky-wschange-'));
  dirs.push(dir);
