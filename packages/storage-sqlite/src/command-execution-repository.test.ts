...
import { CommandExecutionStatus, RiskLevel, WorkspaceChangeStatus } from '@quoky/core';
import type { CommandExecution } from '@quoky/core';
...
async function freshStore(): Promise<SqliteStorageProvider> {
  const dir = mkdtempSync(join(tmpdir(), 'quoky-cmdexec-'));
  dirs.push(dir);
