...
function consumeOsc(input: string, start: number): number {
  ...
  // An unterminated OSC is terminal framing with no reliable content boundary.
  return input.length;
}
...
/**
 * Remove machine-recognizable terminal framing from transient prompt context.
 * This is provider-neutral data hygiene: it does not interpret natural language,
 * deduplicate printable text, or mutate persisted/context source values.
 */
export function normalizePromptContextContent(input: string): string {
  let output = '';
