...
const POLITE_REQUEST_TAIL =
  String.raw`(?:줘|주세요|봐(?:\s*주세요)?|보세요|줄래(?:요)?|주실래요?|주시겠어요|줄\s*수\s*있(?:어(?:요)?|나요)|주실\s*수\s*있(?:나요|어요|습니까))`;
...
const DIRECT_TYPECHECK_REQUEST = new RegExp(
  String.raw`(?:typecheck|타입\s*체크|type\s*check)\s*${INTERVENING_REQUEST_WORDS}(?:해(?:\s*(?:${POLITE_REQUEST_TAIL}))?|하(?:세요|자|라)|부탁해)\s*$`,
  'i',
);
