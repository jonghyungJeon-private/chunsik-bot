...
/**
 * Selects an AiProvider for a capability. This is the concrete expression of
 * "capabilities are above models": the core asks for a CAPABILITY and the
 * router returns whichever AVAILABLE provider advertises the highest priority
 * for it. No concrete CLI name appears here — the fallback policy lives in the
 * priorities each provider advertises (see AiCapabilityDescriptor docs).
 */
export class CapabilityRouter {
  constructor(private readonly manager: AiProviderManager) {}

  async route(capability: Capability): Promise<AiProvider> {
