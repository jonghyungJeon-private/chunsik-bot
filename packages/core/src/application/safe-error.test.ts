...
describe('safe-error (Sprint 4c-Follow-up-7, F7-D)', () => {
  ...
  it('formatSafeErrorText (CONFIRMED_NOT_APPLIED) includes the failure statement, mapped message, the confirmed no-change line, and code — and NOT the conservative wording', () => {
    const text = formatSafeErrorText(
      { code: 'TASK_TRANSITION_ERROR', message: '작업 상태를 변경하는 과정에서 허용되지 않은 상태 전이가 발생했어요.' },
    ...
    // the two outcomes cannot be confused: the "cannot verify" wording must be absent when non-mutation is proven
  ...
  it('formatSafeErrorText (MAY_HAVE_APPLIED) uses the conservative "cannot verify" wording and NEVER the confirmed no-change line', () => {
    const text = formatSafeErrorText(
      { code: 'INTERNAL_ERROR', message: '알 수 없는 내부 오류가 발생했어요.' },
    ...
    expect(text).toContain('오류 코드: INTERNAL_ERROR');
    // the two outcomes cannot be confused: the confirmed zero-mutation claim must be absent when uncertain
    expect(text).not.toContain('아직 어떤 변경도 적용되지 않았어요.');
  });

  it('formatSafeErrorText DEFAULTS to the conservative MAY_HAVE_APPLIED wording when mutationSafety is omitted (no accidental false zero-mutation claim)', () => {
    const text = formatSafeErrorText({ code: 'INTERNAL_ERROR', message: '알 수 없는 내부 오류가 발생했어요.' });
    expect(text).toContain('변경 적용 여부를 확인할 수 없어요.');
    expect(text).toContain('추가 작업을 진행하기 전에 현재 상태를 확인해주세요.');
    expect(text).not.toContain('아직 어떤 변경도 적용되지 않았어요.');
  });
  ...
  it('includes optional non-secret stage + requestId when provided', () => {
    const text = formatSafeErrorText(
      ...
      { stage: 'intent-routing', requestId: 'req-ab12cd', mutationSafety: 'CONFIRMED_NOT_APPLIED' },
    );
